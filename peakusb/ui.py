...
class PeakUSBApp(tk.Tk):
    def __init__(self):
        ...
        self.expected_checksum = tk.StringVar()
        ...
        self.windows_to_go_mode = tk.BooleanVar(value=False)
        ...
        self._configure_filesystem_options()
    ...
    def _configure_dpi_scaling(self):
        ...
        try:
            ...
            scale = max(1.0, pixels_per_inch / 72.0)
            self.tk.call("tk", "scaling", scale)
        except Exception:
            pass

    def _configure_filesystem_options(self):
        values = backend.get_supported_filesystems()
        self.fs_combo["values"] = values
        if self.fs_type.get() not in values:
            self.fs_type.set(values[0] if values else "NTFS")
    ...
    def create_widgets(self, parent):
        ...
        ttk.Label(iso_container, text="Expected SHA256 (optional):").grid(row=1, column=0, sticky="e", padx=6, pady=2)
        checksum_entry = ttk.Entry(iso_container, textvariable=self.expected_checksum, width=30)
        checksum_entry.grid(row=1, column=1, sticky="we", padx=6, pady=2)
        ttk.Button(
            iso_container,
            text="Load",
        ...
        self.fs_combo.grid(row=0, column=1, sticky="we", padx=4, pady=2)
    ...
    def browse_checksum_file(self):
        path = filedialog.askopenfilename(
            ...
            filetypes=[("Checksum files", "*.sha256 *.sha *.txt"), ("All files", "*")],
        )
        if not path:
            return

        try:
            checksum = backend.load_expected_checksum(path)
            self.expected_checksum.set(checksum)
        except Exception as e:
            messagebox.showerror("Checksum File Error", str(e))
    ...
    def start_process(self):
        ...
        try:
            self.progress_text.config(text="Detecting Windows ISO compatibility...")
            is_wtg_compatible = backend.is_windows_to_go_compatible_iso(iso)
        except Exception:
            is_wtg_compatible = False

        if is_wtg_compatible:
            choice = messagebox.askyesnocancel(
                "Windows ISO Detected",
                "This ISO looks compatible with Windows To Go.\n\n"
                "Yes: Create Windows To Go (persistent data partition)\n"
                "No: Create standard Windows bootable USB\n"
                "Cancel: Stop",
            )
            if choice is None:
            ...
            if choice:
                self.windows_to_go_mode.set(True)
                self.create_persistent.set(True)
                self.fs_type.set("NTFS")
                self.volume_label.set("WINTOGO")
            else:
                self.windows_to_go_mode.set(False)
                self.create_persistent.set(False)
        else:
            self.windows_to_go_mode.set(False)
    ...
    def _run_process_thread(self):
        try:
            ...
            self._update_status(f"Formatting device as {fs.upper()}...")
            backend.format_device(dev, fs_type=fs, label=label, quick_format=self.quick_format.get())

            expected_checksum = backend.normalize_checksum(self.expected_checksum.get())
            if expected_checksum and len(expected_checksum) != 64:
                raise Exception("Expected SHA256 must be a 64-character hexadecimal hash, or leave it blank.")

            if self.verify_checksum.get() and expected_checksum:
                self._update_status("Validating ISO checksum...")
                actual_checksum = backend.verify_checksum(iso, "sha256")
                if actual_checksum.lower() != expected_checksum:
                    raise Exception(
                        "ISO checksum mismatch.\n"
                        f"Expected: {expected_checksum}\n"
                        f"Actual:   {actual_checksum.lower()}"
                    )
            ...
            if self.windows_to_go_mode.get():
                self._update_status("Creating Windows To Go data partition...")
                backend.enable_persistence(dev, size_mb=16384, label="WTG_DATA")
            elif self.create_persistent.get():
                self._update_status("Creating persistent partition...")
                backend.enable_persistence(dev, size_mb=4096, label="PERSISTENT")
                ...
                if expected_checksum:
                    self.after(
                        ...
                        lambda: messagebox.showinfo(
                            "Verification",
                            "ISO checksum validation passed.\n"
                            f"SHA256: {checksum}",
                        ),
                    )
                else:
